...
layout(location = 2) in vec4 inNormalSign; // snorm8: xyz normal, w tangent sign
layout(location = 3) in vec4 inTangent;    // snorm8
layout(location = 4) in vec4 inColor;      // unorm8
...
void main()
{
    ...
    outNormal     = normalize(mat3(model) * inNormalSign.xyz);
    outTangent    = vec4(normalize(mat3(model) * inTangent.xyz), inNormalSign.w);
    outUV         = inUV;
    outColor      = inColor;
