...
inline S_Quat::S_Quat(const float *i_array)
{
    m_data = glm::make_quat( i_array );
}

inline S_Quat::S_Quat(const double *i_array)
{
    m_data = glm::make_quat( i_array );
}

inline S_Quat::S_Quat(const S_Quat &i_q) : m_data( i_q.m_data )
{
...
{
    this->m_data = i_q.m_data;
    return *this;
}

inline bool S_Quat::operator==(const S_Quat &i_q)
{
    return ( m_data == i_q.m_data );
}
